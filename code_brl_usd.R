...
#Load data
	#USD/BRL data
		data=read.csv("exchange_rate_USD_BRL_daily.csv",header=TRUE)
		data$Date=as.POSIXct(data$Date,format='%m/%d/%Y')  #POSIXct -- calender dates and times
		data=xts(data[,2],data[,1]) #xts converts raw data to time series
		colnames(data)="rate"
######################################################################################
